...
const Signage = ({ baseURL }) => {
  ...
  const getDatas = () => {
    ...
    axios
      ...
      .catch((err) => {
        console.log(err);
        setDatas(MOCK_DATA); // エラーが発生した場合、サンプルデータを使用
      });
  };
  ...
  // ─── サンプルデータ（バックエンドが使えないときのフォールバック） ───────────────
  const MOCK_DATA = {
    1: {
      id: 1,
      titles: "サンプル投稿 1",
      comments: "バックエンドなしで表示しているサンプルです。",
      images_url: "https://picsum.photos/seed/sign1/800/600",
      nickname: "サンプルユーザー",
      likeCounts: 12,
    },
    2: {
      id: 2,
      titles: "サンプル投稿 2",
      comments: "複数のスライドを確認できます。",
      images_url: "https://picsum.photos/seed/sign2/800/600",
      nickname: "テストユーザー",
      likeCounts: 7,
    },
    3: {
      id: 3,
      titles: "サンプル投稿 3",
      comments: "スライドショーのデモ画面です。",
      images_url: "https://picsum.photos/seed/sign3/800/600",
      nickname: "デモユーザー",
      likeCounts: 3,
    },
  };
...
const NextSlide = styled.div`
  ...
  background-color: ${({ state }) => (state === 1 ? "#8AAADB" : "#8AAADB")};
`;
const NNextSlide = styled.div`
  ...
  top: 0px;
  border-radius: 0px 0px 2rem 2rem;
  background-color: ${({ state }) => (state === 1 ? "#5D8EDA" : "#5D8EDA")};
`;
